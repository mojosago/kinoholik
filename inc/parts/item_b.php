 <article class="item" id="post-<?php the_id(); ?>">
	<div class="image">
		...
		<?php if($mostrar = $terms = strip_tags( $terms = get_the_term_list( $post->ID, ''))) {  ?><span class="quality"><?php echo $mostrar; ?></span><?php } ?>

		<span class="item_type"><?php if($d = get_post_type() == 'movies') { _e('Филм','mtms'); } if($d = get_post_type() == 'tvshows') { _e('TV','mtms'); } ?><span>

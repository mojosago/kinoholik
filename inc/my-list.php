...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = __( 'Моят списък', 'mtms' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = __( 'В моят списък', 'mtms' );
		$icon = $icon_empty;
	}
...
function show_user_likes( $user ) { ?>        
	<div class="like_box">
		<ul>
			...
			<?php #echo info_movie_get_meta('ids'); ?>
			<li><a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>"><?php the_title(); ?></a></li>
			<?php endwhile; ?>
			<?php else : ?>
			<li><?php _e( 'No content on your list.', 'mtms' ); ?></li>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>

<?php
/**
 * Plugin Name: Sidebar content Most views
 * Description: Sidebar content Most views
 * Version: 1.0
 * Author: Erick Meza
...
class DT_Widget_views extends WP_Widget {
	function DT_Widget_views() {
		$widget_ops = array( 'classname' => 'doothemes_widget', 'description' => __('A widget to display Popular content', 'mtms') );
		$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'dtw_content_views' );
		$this->WP_Widget( 'dtw_content_views', __('DooPlay - Sidebar Popular content', 'mtms'), $widget_ops, $control_ops );
	}
...
echo '</div>';
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'dt_key' ); ?>"><?php _e('Popular content by', 'mtms'); ?></label>
			<select id="<?php echo $this->get_field_id( 'dt_key' ); ?>" name="<?php echo $this->get_field_name( 'dt_key' ); ?>" style="width:100%;">
				<option <?php if ( 'dt_views_count' == $instance['dt_key'] ) echo 'selected="selected"'; ?> value="dt_views_count"><?php _e('Visits','mtms'); ?></option>
				<option <?php if ( '_user_liked' == $instance['dt_key'] ) echo 'selected="selected"'; ?> value="votes_count"><?php _e('Likes','mtms'); ?></option>
				<option <?php if ( '_starstruck_avg' == $instance['dt_key'] ) echo 'selected="selected"'; ?> value="_starstruck_avg"><?php _e('Users rating','mtms'); ?></option>
				<option <?php if ( 'imdbRating' == $instance['dt_key'] ) echo 'selected="selected"'; ?> value="imdbRating"><?php _e('IMDb rating','mtms'); ?></option>
				<option <?php if ( 'vote_average' == $instance['dt_key'] ) echo 'selected="selected"'; ?> value="vote_average"><?php _e('TMDb rating','mtms'); ?></option>
            </select>
		</p>

	...
	<tbody>
		...
		<tr id="ids_box">
			...
			<td class="field">
				...
				<p class="description"><?php _e('E.g. https://www.themoviedb.org/tv/<strong>1402</strong>-the-walking-dead/season/<strong>1</strong>/episode/<strong>2</strong>','mtms'); ?></p>
			</td>
		</tr>
		<?php } ?>
		<tr id="episode_name_box">
			<td class="label">
				<label><?php _e('Episode title','mtms'); ?></label>
			</td>
			<td class="field">
		...
		<tr id="dt_backdrop_box">
			<td class="label">
				<label><?php _e( 'Main Backdrop', 'mtms' ); ?></label>
				<p class="description"><?php _e('Add url image','mtms'); ?></p>
			</td>
			<td class="field">
		...
		<tr id="imagenes_box">
			<td class="label">
				<label><?php _e( 'Backdrops', 'mtms' ); ?></label>
            ...
            jQuery.getJSON(url + input + "?" + idioma + apikey, function(tmdbdata) {
                jQuery.each(tmdbdata, function(key, item) {
                    if (key == "name") {
						...
						jQuery('input[name=post_title]').val(item + " " + temp + "x" + epis);
                    }
                });
            });

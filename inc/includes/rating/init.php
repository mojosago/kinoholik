...
function starstruck_action_links( $links ) {
	$action_links = array(
		'settings' => '<a href="' . admin_url( 'options-general.php?page=stars-rating' ) . '" title="' . esc_attr( __( 'View Starstruck Settings', 'mtms' ) ) . '">' . __( 'Settings', 'mtms' ) . '</a>',
	);
...
function starstruck_return_content_span( $id, $rating, $type, $class = 'starstruck-main' ) {
	return '<span class="starstruck ' . esc_attr( $class ) .' " data-id="' . esc_attr( $id ) . '" data-rating="' .esc_attr( $rating ) . '" data-type="' .esc_attr( $type ) . '"></span>';
}
...
function starstruck_text( $data, $id ) {
	...
	$pre   = '<span>';
	$post  = '</span>';
	$text  = __( 'Thanks for your vote!', 'mtms' );
	$data  = maybe_unserialize( $data );
	$match = array_key_exists( $user_info->ID, $data['users'] ) ? true : false;
	$vote_text = _n( 'vote', 'votes', $data['votes'], 'mtms' );
	if ( $match == true ) {
		return $output = array(
			'text'      => $pre . $text . $post,
			'votes'     => $data['votes'],
...
function starstruck_action_callback() {
	if ( ! wp_verify_nonce( $_POST['nonce'], 'starstruck-nonce' ) ) {
		header( __( 'HTTP/1.1 400 Nonce is empty and/or invalid', 'mtms' ) );
	}
	if ( ! isset( $_POST['id'] ) ) {
		header( __( 'HTTP/1.1 400 ID is empty', 'mtms' ) );
	} else {
		$id = $_POST['id'];
	}
	if ( ! isset( $_POST['type'] ) ) {
...
function starstruck_delete_action_callback() {
	...
	$response = array( 'text' => __( 'User vote deleted', 'mtms' ) );
	header( 'Content-Type: application/json' );
	echo json_encode( $response );
	die();
}
...
function starstruck_user_object() {
	...
	if ( $user_ID > 0 ) {
		...
		$user_info->display_name = __( 'Anonymous', 'mtms' );
	}
	return $user_info;
}
...
function starstruck_get_top_ratings( $type = 'post', $post_type = 'post', $limit = 10 ) {
	...
	if ( $ratings ) {
		...
		?>
				<li>
					<div class="starstruck-wrap">
						...
						<div class="starstruck-rating"><?php _e( 'Rating:', 'mtms' ); ?> <?php echo $rating; ?> - <?php echo number_format( $votes ); ?> <?php echo _n( 'vote', 'votes', $votes, 'mtms' ); ?></div>
					</div><!-- .starstruck-wrap -->
				</li>
		<?php
			$index++;
		...
		echo '<p>' . __( 'No results found.', 'mtms' ) . '</p>';
	}
	$top .= ob_get_clean();
	return $top;
}

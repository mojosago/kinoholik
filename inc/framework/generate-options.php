...
public function acera_add_menu() {
		add_theme_page(__(MENU_TITLE), __(MENU_TITLE), 'administrator',  MENU_SLUG, array(&$this, 'acera_display_page'), 'dashicons-admin-tools');
}
   ...
   jQuery('#acera-settings').submit(function() { 
      jQuery(this).ajaxSubmit({
         success: function(){
            ...
            jQuery('.saveMessage').append("<div id=\"message\" class=\"notice rebskt updated \"><p><?php echo htmlentities(__('Settings saved.','mtms'),ENT_QUOTES); ?></p></div>").show();
         }, 
         timeout: 5000
      }); 
...
<p><?php _e('Invalid license, it is possible that some of the options may not work correctly','mtms') ?> <a href="<?php echo admin_url(); ?>themes.php?page=<?php echo DT_THEME_SLUG; ?>-license"><?php _e('here','mtms'); ?></a></p>
</div>
<?php endif ?>
...
<div id="acera-meta-info">
<h2><a href="<?php echo DT_SUPPORT_FORUMS; ?>" target="_blank"><?php _e('Support forum','mtms'); ?></a></h2>
<h2><a href="<?php echo DT_CHANGELOG; ?>" target="_blank"><?php _e('Changelog','mtms'); ?></a></h2>
</div>
</div>
...
<div class="social_single_left">
	<ul class="lsocials">
		...
		<li><a class="support" href="https://doothemes.com/cpa/support/?licencia=<?php echo get_option('dooplay_license_key'); ?>&theme=<?php echo DT_THEME_NAME; ?>&version=<?php echo DT_VERSION; ?>&url=<?php echo dt_domain(get_option('siteurl')); ?>" target="_blank" onClick="window.open(this.href, this.target, 'width=600,height=650'); return false;" target="_blank"><i class="dashicons 
dashicons-admin-comments"></i></a></li>
	</ul>
</div>
...
<input name="publish" class="dt_button button button-primary button-large" type="submit" value="<?php _e('Save changes','mtms'); ?>" id="publish" />
</div>
<?php
...
<input name="save" class="dt_button button button-primary button-large" type="submit" value="<?php _e('Save changes','mtms'); ?>" />
</div>
</div> 
...
<span class="upload acera_upload acera-button-blue" id="<?php echo $value['id']; ?>"><?php _e('Upload','mtms'); ?></span>
<?php if (get_option($value['id'])): ?>
<span type="button" class="acera_remove acera-button" id="remove_<?php echo $value['id']; ?>"><?php _e('Remove','mtms'); ?></span>
<?php endif; ?>
